...
int main(int argc, char** argv) {
    ...
    Item basketball("Basketball", "LeBron's favorite ball", 40, 2.f);
    Item proteinShake("Protein Shake", "A delicious chocolate bar", 250, 0.2f);
    Item championshipRing("Championship Ring", "A shimmering ring symbolizing one of LeBron's NBA titles. It glows with a legendary aura.", 0, 0.1f);
    Item sneakers("Nike LeBron Sneakers", "A pair of iconic Nike LeBron 20s. Wearing them might help you move faster on and off the court.", 0, 0.5f);

    game.curr_location.add_item(basketball);
    game.curr_location.add_item(proteinShake);
    game.curr_location.add_item(championshipRing);
    game.curr_location.add_item(sneakers);    
    return 0;
}